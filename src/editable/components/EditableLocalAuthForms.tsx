...
const inputClass = 'w-full rounded-full border border-[var(--editable-border)] bg-[var(--slot4-panel-bg)] px-5 py-4 text-sm font-medium text-[var(--slot4-page-text)] outline-none transition placeholder:text-[var(--slot4-soft-muted-text)] focus:border-[var(--slot4-accent)] focus:bg-[var(--slot4-surface-bg)]'
const buttonClass = 'inline-flex w-full items-center justify-center rounded-full bg-[var(--slot4-accent-fill)] px-6 py-4 text-sm font-semibold text-[var(--slot4-on-accent)] transition hover:brightness-105 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-[1.1rem] px-5 py-3.5 text-[13.5px] font-medium leading-6 ${status === 'success' ? 'bg-emerald-50 text-emerald-800' : 'bg-[var(--slot4-accent-soft)] text-[var(--slot4-accent)]'}`}>{message}</p> : null}
